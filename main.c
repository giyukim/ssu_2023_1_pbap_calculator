...
int eval_expression(char[MAX_STRING], int[VARIABLE_COUNT][BINT_ARR_LEN], int[HISTORY_VARIABLE_COUNT][BINT_ARR_LEN], char[MAX_STRING], int[1]);

// 메인 함수에서 반복문 안에 들어가는 함수.
// 종료 시 (q 커맨드 등) LOOP_STOP 리턴. 아니면 LOOP_CONTINUE 리턴
int screen_iteration(int[VARIABLE_COUNT][BINT_ARR_LEN], int[HISTORY_VARIABLE_COUNT][BINT_ARR_LEN], int[MAX_LINE_COUNT], char[MAX_LINE_COUNT][MAX_STRING]);
...
// 새로운 파일로 출력하는 함수
int file_io_write_new(char [MAX_FILENAME_STRING], char [][MAX_STRING], int);
...
// History (H/h) 커맨드
void command_history(int[HISTORY_VARIABLE_COUNT][BINT_ARR_LEN], int[MAX_LINE_COUNT], char[MAX_LINE_COUNT][MAX_STRING]);
...
// Reset   (T/t) 커맨드
void command_reset(int[VARIABLE_COUNT][BINT_ARR_LEN], int[HISTORY_VARIABLE_COUNT][BINT_ARR_LEN], int[MAX_LINE_COUNT]);
...
int main() {
    ...
    while(1) if(screen_iteration(variables, history_variables, line_types, lines) == LOOP_STOP) break;
    ...
    for(int raw_pointer = 0; raw_pointer < count_exp_raw; raw_pointer++) {
        ...
        switch(current_token_type) {
            ...
            case TOKEN_INTEGER:
                ...
                if(big_int_parse(exp_raw, integer_start, integer_end, exp_tokens_integer[count_exp_tokens_integer]) == FAIL) return FAIL;
...
int screen_iteration(int variables[VARIABLE_COUNT][BINT_ARR_LEN], int history_variables[HISTORY_VARIABLE_COUNT][BINT_ARR_LEN], int line_types[MAX_LINE_COUNT], char lines[MAX_LINE_COUNT][MAX_STRING])
{
...
int big_int_lcomb(int a, int x[BINT_ARR_LEN], int b, int y[BINT_ARR_LEN], int result[BINT_ARR_LEN])
{
    ...
    if(big_int_fix(carry, temp_result) == FAIL) return FAIL;
...
void draw_main(int variable[VARIABLE_COUNT][BINT_ARR_LEN], int line_types[MAX_LINE_COUNT], char lines[MAX_LINE_COUNT][MAX_STRING]){
    printf("************************************************************************\n");
    printf("                               큰정수계산기\n");
    printf("************************************************************************\n");
    printf("* 변 수\n");
...
{
    for(int i = 0; i < MAX_LINE_COUNT; i++) lines_type[i] = LINE_TYPE_NOTHING;
}
...
int calculate_groups(int exp_tokens[MAX_TOKEN_COUNT][2], int exp_tokens_integer[MAX_TOKEN_COUNT][BINT_ARR_LEN], int variables[VARIABLE_COUNT][BINT_ARR_LEN], int history_variables[HISTORY_VARIABLE_COUNT][BINT_ARR_LEN], int exp_groups[MAX_GROUP_COUNT][3], int exp_groups_result[MAX_GROUP_COUNT][BINT_ARR_LEN])
{
    ...
    for(int group_index = 0; group_index < group_len; group_index++) {
        ...
        if(group_numeral == NUMERAL_UNARY) {
            ...
            if(unary_operator_type == OPERATOR_UNARY_MINUS) if(big_int_times(-1, result_big_int, result_big_int) == FAIL) return FAIL;
        }
